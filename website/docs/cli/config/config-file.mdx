...
extensions. OpenTofu will not recognize a file named `opentfc.rc.txt` as a
...
If you are running the OpenTofu CLI interactively on a computer with a web browser, you can use [the `opentf login` command](/docs/cli/commands/login)
