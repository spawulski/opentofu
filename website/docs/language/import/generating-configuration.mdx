...
Configuration generation is available in OpenTofu v1.6 as an experimental feature. Later minor versions may contain changes to the formatting of generated configuration and behavior of the `opentf plan` command using the `-generate-config-out` flag.
:::
...
To generate configuration, run `opentf plan` with the `-generate-config-out` flag and supply a new file path. Do not supply a path to an existing file, or OpenTofu throws an error.
...
To instruct OpenTofu to generate configuration for the `import` blocks you defined, run `opentf plan` with the `-generate-config-out=` flag and a new file path. OpenTofu displays its plan for importing your resource and the file where OpenTofu generated configuration based on this plan.
